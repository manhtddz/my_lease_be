...
class NewStorage
{
    ...
    {
        if (!$this->_isUploadFile($content)) {
            $content = $this->_base64ToFile($content);
        }
    ...
    public function uploadToTmp($content, $fileName = null)
    {
        ...
        if (!$this->_validationFile($fileName, $content)) {
            return false;
        }

        $newFilePath = getTmpUploadDir(date('Y-m-d')) . '/' . $fileName;
        $this->_deleteTmpDaily();
    ...
    public function uploadFileToMedia($file, string $fileName = '', array $options = ['public-read'])
    {
        ...
        if (!$this->_validationFile($fileName, $file)) {
            $result['status'] = false;
            return $result;
        }

        $newFileSavePath = getMediaDir($fileName);
        if ($this->_isUploadFile($file)) {
     ...
     * @return bool
     */
    protected function _isUploadFile($data)
    {
     ...
     * @return false|string
     */
    protected function _base64ToFile($fileData)
    {
    ...
    protected function _validationFile($fileName, $content)
    {
        ...
        if (in_array($ext, $extBlacklist)) {
            logError(__('messages.file_upload_blacklist') . PHP_EOL . "(File: " . $fileName . ", Blacklist: " . jsonEncode($extBlacklist) . ")");
            return false;
        }
     ...
     * delete tmp file
     */
    protected function _deleteTmpDaily()
    {
