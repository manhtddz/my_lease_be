...
trait BaseSoftDelete
{
    ...
    protected function runSoftDelete()
    {
        ...
        if (!empty($this->getDeletedByColumn())) { // deleted_by
            $this->{$this->getDeletedByColumn()} = getLoginUser($this->getKeyName());
            $columns[$this->getDeletedByColumn()] = getLoginUser($this->getKeyName());
        } else if (!empty($this->getUpdatedByColumn())) { // updated_by
            $this->{$this->getUpdatedByColumn()} = getLoginUser($this->getKeyName());
            $columns[$this->getUpdatedByColumn()] = getLoginUser($this->getKeyName());
        }

        // deleted_at + updated_at
        if (!empty($this->getDeletedAtColumn())) { // deleted_at
            $this->{$this->getDeletedAtColumn()} = $time;
            $columns[$this->getDeletedAtColumn()] = $this->fromDateTime($time);
        } else if (!empty($this->getUpdatedAtColumn())) { // updated_at
    ...
    public function restore(): bool|null
    {
        ...
        if (!empty($this->getDeletedAtColumn())) { // deleted_at
            ...
            $this->{$this->getUpdatedAtColumn()} = $this->fromDateTime($time);
        }
     ...
     * @return string|null
     */
    public function getQualifiedDeletedFlag(): string|null
    {
