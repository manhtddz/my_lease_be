...
class PushNotification
{
     ...
     */
    public function getInfo($token): mixed
    {
        if (empty($token)) {
            return false;
        }
     ...
     * @throws GuzzleException
     */
    public function subscribeTopic($token, $topic): mixed
    {
     ...
     * @throws GuzzleException
     */
    public function unsubscribeTopic($token, $topic): mixed
    {
        if (empty($token) || empty($topic)) {
            return false;
        }
     ...
     * @throws GuzzleException
     */
    public function sendNotificationToDevices($title, $message, $tokens): bool|array
    {
        if (empty($tokens)) {
            return false;
        }
        ...
        foreach ($tokens as $token) {
            ...
            $logs = jsonEncode(['url' => $url, 'params' => $params, 'method' => 'POST', 'headers' => $this->headers]);
            ...
            if (!isset($response['result'])) {
                ...
                $msg = str(__('messages.push_messages_error'))->append(
                    ...
                    'Request: ' . $logs,
                    PHP_EOL,
                    'Response: ' . jsonEncode((array)$response),
                );
                logError($msg);
                continue;
            }
     ...
     * @throws GuzzleException
     */
    public function sendNotificationToTopic($title, $message, $topics): bool|array
    {
        if (empty($topics)) {
            return false;
        }
     ...
     * @throws GuzzleException
     */
    protected function _callApi($url, array $params = [], string $method = 'POST'): mixed
    {
        $logs = jsonEncode(['url' => $url, 'params' => $params, 'method' => $method, 'headers' => $this->headers]);
